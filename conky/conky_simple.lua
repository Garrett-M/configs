...
function conky_main()
  ...
  cairo_move_to(cr, 0.4525 * WINDOW_WIDTH, 0.3 * WINDOW_HEIGHT)
  cairo_show_text(cr, conky_parse("${time %H:%M}"))
  cairo_stroke(cr)
  
  -- DATE
  cairo_set_font_size(cr, 0.016 * WINDOW_WIDTH)
  cairo_move_to(cr, 0.4625 * WINDOW_WIDTH, 0.347 * WINDOW_HEIGHT)
  local time_str = string.format('%-11s', conky_parse("${time %A,}"))..conky_parse("${time %b %d, %Y}")
  ...
  local cx,cy = 0.282 * WINDOW_WIDTH, 0.652 * WINDOW_HEIGHT
  ...
  local ps_str = conky_parse("${exec ps -Ao comm,pcpu,pmem  --sort=-pmem | head -n 15}")
  local processes = {}
  for line in string.gmatch(ps_str, '([^\n]+)') do
    table.insert(processes, line)
  end
  for line = 1,table.getn(processes) do
    cairo_move_to(cr, 0.243 * WINDOW_WIDTH, 0.643 * WINDOW_HEIGHT + line * 0.014 * WINDOW_HEIGHT)
  ...
  local rows = 4
  local perc = 0.0
  local perc_incr = 100.0 / 32.0
  local cx,cy = 0.4935 * WINDOW_WIDTH, 0.598 * WINDOW_HEIGHT
...
function conky_fs_main()
  ...
  local offset = 0.785 * WINDOW_WIDTH
